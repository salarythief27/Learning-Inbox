...
情報を役割ごとに整理し、ダッシュボードで学習状況を振り返れます。

## 開発目的

PythonとFlaskを使ったWebアプリ開発の流れを理解し、就職活動用ポートフォリオとして説明できるアプリを作ることを目的に開発しました。
...
- ログイン中のユーザー以外のデータへアクセスさせない条件の統一
- プロジェクト削除と、所属タスクを未所属へ変更する処理を同じトランザクションで行うこと
- タスクの状態と完了日の整合性をPython側でも保つこと
- ギモンを解決済みにする場合だけ回答を必須にする、項目間のバリデーション
- 検索条件の値を検証しながら、検索後も画面へ選択状態を保持すること
- Windows環境でテスト用SQLite DBの接続を閉じてから後片付けすること
